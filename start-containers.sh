...
# mongo_redis_net network
docker run -i -t -d -p 6379:6379 --name redis_server --network=mongo_redis_net my_redis_cache
...
docker run -i -t -d -p 80:80 --name python_app --network=mongo_redis_net my_mongo_redis
